...
if ($id == 0) {
    ...
    mysqli_stmt_bind_param($comando, 'ssdssis', $nome, $descricao, $preco, $novo_nome, $estoque, $plataforma, $categoria);
} else {
    ...
    if ($novo_nome) {
        $sql = "UPDATE tb_jogos SET 
                ...
                WHERE id = ?";
        $comando = mysqli_prepare($conexao, $sql);
        mysqli_stmt_bind_param($comando, 'ssdssii', $nome, $descricao, $preco, $novo_nome, $estoque, $plataforma, $categoria, $id);
    } else {
        $sql = "UPDATE tb_jogos SET 
                    titulo = ?, 
                    descricao = ?, 
                ...
                WHERE id = ?";
        $comando = mysqli_prepare($conexao, $sql);
        mysqli_stmt_bind_param($comando, 'ssdssi', $nome, $descricao, $preco, $estoque, $plataforma, $categoria, $id);
    }
}
